...
public class RingTimeTest {
    ...
    public void testInvalid(){
        final RingTime invalidRingTime = new RingTime(DateTime.now().plusMinutes(1).getMillis(), DateTime.now().getMillis(), DateTime.now().getMillis());
    }

    @Test
    public void testRegularRingTime(){
        final DateTime ring = DateTime.now();
        final RingTime regularRingTime = new RingTime(ring.getMillis(), ring.getMillis(), DateTime.now().getMillis());
    ...
    public void testSmartRingTime(){
        final DateTime ring = DateTime.now();
        final RingTime smartRingTime = new RingTime(ring.minusMinutes(1).getMillis(), ring.getMillis(), DateTime.now().getMillis());
