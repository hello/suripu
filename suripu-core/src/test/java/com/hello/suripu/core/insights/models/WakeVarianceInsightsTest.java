...
public class WakeVarianceInsightsTest {
    ...
    @Test
    public void testNoCardGenerated2() {
        final Long accountId = 999L;
        final List<Integer> wakeTimeList = new ArrayList<>();
        wakeTimeList.add(-1);
        final Optional<InsightCard> noResult = WakeVariance.processWakeVarianceData(accountId, wakeTimeList, new WakeStdDevData());
        ...
        final ImmutableList<Integer> availList = ImmutableList.copyOf(ContiguousSet.create(Range.closed(0, 170), DiscreteDomain.integers()));
        final WakeStdDevData wakeStdDevData = new WakeStdDevData();
        for (Integer stddev : availList) {
            final Integer percentile = wakeStdDevData.getWakeStdDevPercentile(stddev);
            assertThat(percentile >= 0, is(Boolean.TRUE));
        }
    }
