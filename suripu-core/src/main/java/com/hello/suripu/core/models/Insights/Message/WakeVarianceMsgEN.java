...
public class WakeVarianceMsgEN {
    public static Text getWakeVarianceLow(final int wakeStdDev, final int percentile) {
        return new Text("Hello, very regular",
                String.format("The time you wake up each morning is **very consistent**, which is great. It varied an average of %d minutes last week, ", wakeStdDev) + String.format("Which is lower than %d%% of all Sense users. ", 100 - percentile) +
    ...
    public static Text getWakeVarianceHigh(final int wakeStdDev, final int percentile) {
        return new Text("Hello, irregular",
                String.format("The time you wake up each morning is **a little inconsistent**. It varied an average of %d minutes last week, ", wakeStdDev) + String.format("which is higher than %d%% of all Sense users. ", percentile) +
                        "\n\nWaking up at the same time each morning is great for your internal clock, which helps you get better sleep.");
    }

    public static Text getWakeVarianceTooHigh(final int wakeStdDev, final int percentile) {
        return new Text("Hello, very irregular",
                String.format("The time you wake up each morning is **pretty inconsistent**. It varied an average of %d minutes last week, ", wakeStdDev) + String.format("which is higher than %d%% of all Sense users.", percentile) +
                        "\n\nWaking up at the same time each morning is great for your internal clock, which helps you get better sleep.");
    }
}