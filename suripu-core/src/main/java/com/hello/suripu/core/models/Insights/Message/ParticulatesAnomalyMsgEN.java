...
public class ParticulatesAnomalyMsgEN {
    ...
    public static Text getAirImprovement(final int currentDust, final int historyDust, final int percent) {
        return new Text("Improved", String.format("The air quality in your bedroom **improved** by %d%% yesterday, from %dµg/m³ to %dµg/m³.", percent, historyDust, currentDust));
    }

    public static Text getAirWorse(final int currentDust, final int historyDust, final int percent) {
        return new Text("Worse", String.format("The air quality in your bedroom **worsened** by %d%% yesterday, from %dµg/m³ to %dµg/m³.", percent, historyDust, currentDust));
    }
