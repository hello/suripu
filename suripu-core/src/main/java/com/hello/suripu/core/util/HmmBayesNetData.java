...
import com.hello.suripu.core.models.BetaDistributionModel;
...
public class HmmBayesNetData {
    ...
    * */
    ...
    public void updateModelPriors(final List<BayesNetHmmModelPrior> priors) {
        ...
        for (final BayesNetHmmModelPrior prior : priors) {
            ...
            if (joinerOfMultipleHmmOutputs.hasModel(prior.modelId)) {
                ...
                final List<ModelWithDiscreteProbabiltiesAndEventOccurence> newBetaBinomials = getConditionalProbabilityModel(prior.priors);
    ...
    private  List<ModelWithDiscreteProbabiltiesAndEventOccurence> getConditionalProbabilityModel(final List<BetaDistributionModel> priors) {
        List<ModelWithDiscreteProbabiltiesAndEventOccurence> condProbModel = Lists.newArrayList();
        for (final BetaDistributionModel model : priors) {

            //create binary complementary model (p, not p) and you can do this with the beta distribution by swapping alpha and beta
            final BetaBinomialBayesModel betaBinomialBayesModel = new BetaBinomialBayesModel(model.alpha,model.beta);
            final BetaBinomialBayesModel betaBinomialBayesModelComplementary = new BetaBinomialBayesModel(model.beta,model.alpha);
