...
public interface DeviceDAOAdmin extends Transactional<DeviceDAOAdmin> {
    ...
    @SqlQuery("SELECT id, pill_id, fw_version as firmware_version, battery_level, last_updated as last_seen, uptime FROM pill_status WHERE pill_id = :pill_id AND last_updated is not null AND last_updated <= :end_ts ORDER BY id DESC LIMIT 500;")
